...
RESOURCE = os.environ.get(
    ...
    "projects/758182797445/locations/us-east1/reasoningEngines/5816458292376895488",
)
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            if mime == _A2UI_MIME:
                ...
                inner_meta = root.data.get("metadata") or {}
                if isinstance(inner_meta, dict) and inner_meta.get("mimeType") == _A2UI_MIME:
                    out.append({"kind": "a2ui", "data": root.data.get("data", root.data)})
                elif "beginRendering" in root.data or "surfaceUpdate" in root.data:
                    out.append({"kind": "a2ui", "data": root.data})
                elif "text" in root.data:
                    out.append({"kind": "text", "text": str(root.data["text"])})
